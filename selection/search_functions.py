...
def selectTaus(event,selectedTausIdx):
 for itau in range(event.nTau):
  if not event.Tau_pt[itau]>=20: continue
  if not abs(event.Tau_eta[itau])<=2.1: continue
  if not event.Tau_idDecayMode[itau]<5: continue
  if not ord(event.Tau_idDeepTau2017v2p1VSjet[itau])>=16: continue #16 = Medium
  if not ord(event.Tau_idDeepTau2017v2p1VSe[itau])>=2: continue #2 = VVLoose
  if not ord(event.Tau_idDeepTau2017v2p1VSmu[itau])>=1: continue #1 = VLoose
...
def selectEles(event,selectedElesIdx,selectedTausIdx,objectClearningDr):
 ...
 for iele in range(event.nElectron):
  ...
  if not event.Electron_mvaFall17V2Iso_WP90[iele]: continue #consider using it if no corrections for pfRelIso03_all
  ...
  for itau in range(0,len(selectedTausIdx)): 
   if eles[iele].p4().DeltaR(taus[selectedTausIdx[itau]].p4())<=objectClearningDr: 
    isNotTau = False
    break 
  if not isNotTau: continue
...
def selectMus(event,selectedMusIdx,selectedTausIdx,objectClearningDr):
 ...
 for imu in range(event.nMuon):
  ...
  if not ord(event.Muon_pfIsoId[imu])>=2: continue
  isNotTau = True
  for itau in range(0,len(selectedTausIdx)): 
   if mus[imu].p4().DeltaR(taus[selectedTausIdx[itau]].p4())<=objectClearningDr: 
    isNotTau = False
    break 
  if not isNotTau: continue
...
def selectFatJets(event,year,selectedTausIdx,selectedWJetsIdx):
 ...
 for ifatjet in range(event.nFatJet):
  #kinematic
  if not event.FatJet_pt>=200: continue #As we use particleNet, this seems the minimal pT above which SF are measured (see https://twiki.cern.ch/twiki/bin/view/CMS/ParticleNetTopWSFs)
  if not abs(event.FatJet_eta[ifatjet])<2.4: continue
  #ID
  ...
  if year==2016 or year==1016: jetId = 3 #tight ID
  if year==2017: jetId = 2 #tight ID
  if year==2018: jetId = 2 #tight ID
  if not event.FatJet_jetId[ifatjet]>=jetId: continue
  #cleaning
  isNotTau = True
  for itau in range(0,len(selectedTausIdx)):
  ...
  if year==1016: pNetId = 0.94 #Medium ID
  if year==2016: pNetId = 0.93 #Medium ID
  if year==2017: pNetId = 0.94 #Medium ID
  if year==2018: pNetId = 0.94 #Medium ID
  if not event.FatJet_particleNet_WvsQCD[ifatjet]>=pNetId: continue
  #save idx 
...
def selectJets(event,year,selectedTausIdx,selectedWJetsIdx,selectedJetsIdx,selectedTightBIdx,objectClearningDr):
 taus = Collection(event,'Tau')
 fatjets = Collection(event,'FatJet')
 ...
 for ijet in range(0,len(event.nJet)):
  ...
  if year==2017: jetId = 2 #tight ID
  if year==2018: jetId = 2 #tight ID
  if not event.Jet_jetId[ijet]>=jetId: continue
  #cleaning
  isNotTau = True
  for itau in range(0,len(selectedTausIdx)):
  ...
  #b-jet
  bjetEta = -1
  if year==2016: bjetEta = 2.4 
  if year==2017: bjetEta = 2.5 
  if year==2018: bjetEta = 2.5 
  if not abs(event.Jet_eta[ijet])<=bjetEta: continue
  ...
  if year==1016: jetId = 0.6502 #L,M,T: 0.0508,0.2598,0.6502 see https://twiki.cern.ch/twiki/bin/view/CMS/BtagRecommendation106XUL16preVFP   
  if year==2016: jetId = 0.6377 #L,M,T: 0.0480,0.2489,0.6377 see https://twiki.cern.ch/twiki/bin/view/CMS/BtagRecommendation106XUL16postVFP
  if year==2017: jetId = 0.7476 #L,M,T: 0.0532,0.3040,0.7476 see https://twiki.cern.ch/twiki/bin/view/CMS/BtagRecommendation106XUL17
  if year==2018: jetId = 0.7100 #L,M,T: 0.0490,0.2783,0.7100 see https://twiki.cern.ch/twiki/bin/view/CMS/BtagRecommendation106XUL18
  if not event.Jet_btagDeepFlavB[ijet]>=jetId: continue
  #save b idx
