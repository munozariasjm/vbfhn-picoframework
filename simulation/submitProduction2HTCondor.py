...
parser.add_argument(
    ...
    dest="simulationType",
    action="store",
    choices=["FastSimulation", "FullSimulation"],
    type=str,
    default="FullSimulation",
)
parser.add_argument(
    "-s",
    "--step",
    ...
    choices=[
        "gridpack",
        "wmLHEGEN__CMSSW_10_6_22",
        "GEN__CMSSW_10_6_22",
        "SIM__CMSSW_10_6_17_patch1",
        "DIGIPremix__CMSSW_10_6_17_patch1",
        "HLT__CMSSW_10_2_16_UL",
        "RECO__CMSSW_10_6_17_patch1",
        "MiniAOD__CMSSW_10_6_17_patch1",
        "NanoAODv2__CMSSW_10_6_19_patch2",
    ],
    type=str,
    default="gridpack",
)  # Note the steps are sequential from gridpack to NanoAOD, except you have to choose between wmLHEGEN or GEN
parser.add_argument(
    "-gf",
    "--genFragment",
    dest="genFragment",
    type=str,
    default="Hadronizer_TuneCP2_13TeV_generic_LHE_pythia8_cff.py",
)
parser.add_argument(
    "-ne", "--nEvents", dest="nEvents", action="store", type=int, default="10"
)  # not relevant for gridpack
parser.add_argument(
    "-nj", "--nJobs", dest="nJobs", action="store", type=int, default="1"
)  # not relevant for gridpack
parser.add_argument(
    "-um",
    "--ufoModelCase",
    ...
    action="store",
    type=str,
    default="SM_HeavyN_NLO_UFO__mn3",
)  # Note the __ to separate Model and Particle
parser.add_argument(
    "-ac",
    "--archCMSSW",
    dest="archCMSSW",
    action="store",
    type=str,
    default="slc7_amd64_gcc700_CMSSW_10_6_19_tarball.tar.xz",
)
parser.add_argument(
    "-ofp",
    "--outputFilePath",
    ...
    action="store",
    type=str,
    default="/eos/cms/store/group/phys_b2g/azp/hn/simulation/",
)  # gridpackFile = gridpackPath(:outputFilePath+'gridpacks/')+parameters+'_'+archCMSSW
parser.add_argument(
    "-hd",
    "--homeDir",
    dest="homeDir",
    action="store",
    type=str,
    default="/afs/cern.ch/user/f/fromeo/",
)
parser.add_argument(
    "-qh", "--queueHours", dest="queueHours", action="store", type=int, default=24
)
parser.add_argument(
    "-i", "--iterations", dest="iterations", action="store", type=int, default="1"
)
parser.add_argument(
    "-ts", "--timeSleep", dest="timeSleep", action="store", type=str, default="1s"
)
parser.add_argument(
    "-r",
    "--run",
...
campaigns = [
    "RunIISummer20UL18"
]  # RunIISummer20UL16  RunIISummer20UL16APV  RunIISummer20UL17  RunIISummer20UL18 #Not relevant for producing gridpacks
step = args.step
processes = ["WWFusion"]
...
thirdPars = ["none"]
secondParName = (
    "coupling"  # It must be 'none' if such case does not exist in your model
)
secondPars = [0.1, 1]
    ...
    if stepName == "gridpack":
        ...
        for campaign in campaigns:
            ...
            if stepName == "wmLHEGEN":
                previousStepName = (
                    "gridpack"  # not used in this step, so can be any name
                )
                previousStepRootFileName = (
                    "step1_gridpack"  # not used in this step, so can be any name
                )
            elif stepName == "SIM":
            ...
            for process in processes:
                deleteNonBaseFolders(
                    stepName, process
                )  # it deletes folders created in a previous use of this script that are NOT the default ones of the EXO-MCsampleProductions framework, to clear working directory
                production(
                    campaign,
                    process,
                    stepName,
            ...
            os.chdir(
                "../../../../.."
            )  # go back 5 folders, as before os.chdir(simulationDir)
...
def productionGridpack(process):
    ...
    if not os.path.exists("gridpacks"):
        createDir("gridpacks")
    else:
        os.system(
            "rm -r *sh"
        )  # delete previous sh scripts, otherwise they will all be run
    ...
    workingDir = (
        outputFilePath
        + "genproductions/bin/MadGraph5_aMCatNLO/cards/"
        + ufoModelCase
        + "/"
        + process
    )
    os.chdir(
        workingDir
    )  # It assumes you set up outputFilePath as explained in the notes at the beginning of the script
    for tp in thirdPars:
        tp = str(tp).replace(
            ".", "p"
        )  # Need to do it as otherwise the system does not understand the '.' in the code
        for sp in secondPars:
            ...
            for fp in firstPars:
                ...
                fileName = "%s_%s" % (
                    process,
                    parameters,
                )  # Note you need to keep the process in the fileName, even if it seems redundant as it appears also in the folder name where the gridpack is stored, because it is first created in genproductions/bin/MadGraph5_aMCatNLO and if you have different processes with same parameters you might messed up!
                if os.path.exists("%s/%s" % (workingDir, fileName)):
                    os.system(
                        "rm -r %s" % (fileName)
                ...
                # Always delete folders/files before creating them to clean it up
                madDir = (
                    outputFilePath + "genproductions/bin/MadGraph5_aMCatNLO/"
                )  # where the gridpack output (directory,log file,tar file) will be first created (because I do not know how to create the output in a specific folder)
                if os.path.exists("%s/%s" % (madDir, fileName)):
                    os.system("rm -r %s/%s" % (madDir, fileName))  # directory
                if os.path.exists("%s/%s.log" % (madDir, fileName)):
                    os.system("rm -r %s/%s.log" % (madDir, fileName))  # log
                if os.path.exists("%s/%s_%s" % (madDir, fileName, archCMSSW)):
                    os.system(
                        "rm -r %s/%s_%s" % (madDir, fileName, archCMSSW)
                    )  # tar file
                # outputDir is where the files will be at the end of the script
                if os.path.exists("%s/%s" % (outputDir, fileName)):
                    os.system("rm -r %s/%s" % (outputDir, fileName))  # directory
                if os.path.exists("%s/%s.log" % (outputDir, fileName)):
                    os.system("rm -r %s/%s.log" % (outputDir, fileName))  # log
                if os.path.exists("%s/%s_%s" % (outputDir, fileName, archCMSSW)):
                    os.system(
                        "rm -r %s/%s_%s" % (outputDir, fileName, archCMSSW)
                    )  # tar file
                #
                initializeProxy()
                os.system(
                    "mv %s_thirdParZZZ_secondParYYY_firstParXXX_run_card.dat %s_run_card.dat"
                    % (process, fileName)
                )
                os.system(
                    "mv %s_thirdParZZZ_secondParYYY_firstParXXX_extramodels.dat %s_extramodels.dat"
                    % (process, fileName)
                )
                os.system(
                    "mv %s_thirdParZZZ_secondParYYY_firstParXXX_customizecards.dat %s_customizecards.dat"
                    % (process, fileName)
                )
                os.system(
                    "sed -i 's/ZZZ/%s/g' %s_customizecards.dat"
                    % (tp.replace("p", "."), fileName)
                )  # Need to replace back to '.' as we need numbers here
                os.system(
                    "sed -i 's/YYY/%s/g' %s_customizecards.dat "
                    % (sp.replace("p", "."), fileName)
                )
                os.system(
                    "sed -i 's/XXX/%s/g' %s_customizecards.dat"
                    % (fp.replace("p", "."), fileName)
                )
                os.system(
                    "mv %s_thirdParZZZ_secondParYYY_firstParXXX_proc_card.dat %s_proc_card.dat"
                    % (process, fileName)
                )
                os.system(
                    "sed -i 's/thirdPar/%s/g' %s_proc_card.dat"
                    % (thirdParName, fileName)
                )
                os.system("sed -i 's/ZZZ/%s/g' %s_proc_card.dat" % (tp, fileName))
                os.system(
                    "sed -i 's/secondPar/%s/g' %s_proc_card.dat"
                    % (secondParName, fileName)
                )
                os.system("sed -i 's/YYY/%s/g' %s_proc_card.dat" % (sp, fileName))
                os.system(
                    "sed -i 's/firstPar/%s/g' %s_proc_card.dat"
                    % (firstParName, fileName)
                )
                os.system("sed -i 's/XXX/%s/g' %s_proc_card.dat" % (fp, fileName))
                prepareSHFileGridpack(fileName, process)
                os.system(
                    "mv %s.sh %s/gridpacks" % (fileName, path_at)
                )  # need to run from afs according to https://batchdocs.web.cern.ch/troubleshooting/eos.html#no-eos-submission-allowed
                #
                os.chdir("../")
    # create scripts
    ...
    # run on HTcondor
    if run:
        os.system("condor_submit submitTask.cfg")
    # come back to initial folder
    os.chdir(path_at)


def deleteNonBaseFolders(stepName, process):
    if os.path.exists("submit_crab_%s.sh" % (process)):
        os.system("rm submit_crab_%s.sh" % (process))
    if os.path.exists("%s.csv" % (process)):
        os.system("rm %s.csv" % (process))
    if os.path.exists(process) and os.path.isdir(process):
        os.system("rm -r %s" % (process))
    if stepName == "wmLHEGEN":
        if os.path.exists("./Configuration") and os.path.isdir("./Configuration"):
            os.system("rm -r Configuration")
        if os.path.exists("run_cmsdriver_%s.sh" % (process)):
            os.system("rm run_cmsdriver_%s.sh" % (process))
...
def production(campaign, process, stepName, previousStepName, previousStepRootFileName):
    ...
    if stepName == "wmLHEGEN":
        os.system(
            'sed -i \'s/if \\"cvmfs\\" in gridpack:/if "eos" in gridpack:/g\' %s'
            % (configFile)
        )  # Remember we run on HTcondor and have files in eos, not cvmfs
    os.system(
        "python %s %s.csv" % (configFile, process)
    )  # Note that this will create a folder named as the .csv file
    #
    ...
    for tp in thirdPars:
        ...
        for sp in secondPars:
            ...
            for fp in firstPars:
                ...
                evtPerJob = (
                    nEvents / nJobs
                )  # it is ok not to cast, as we want int anyway below
                for j in range(0, nJobs):
                    jobInFileName = (
                        previousStepName + "_" + str(j)
                    )  # not needed in wmLHEGEN step
                    ...
                    if stepName == "wmLHEGEN":
                        ...
                        os.system(
                            "sed -i 's/initialSeed=.*/initialSeed=%s/g' %s/%s"
                            % (newSeed, jobOutFileName, htcName)
                        )  # seed value
                        os.system(
                            "sed -i 's/nEvents = cms\.untracked\.uint32(.*),/nEvents = cms\.untracked\.uint32(%s),/g' %s/%s"
                            % (evtPerJob, jobOutFileName, htcName)
                        )  # nuber of events
                    else:
                        os.system(
                            "sed -i 's/%s.root/%s%s.root/g' %s/%s"
                            % (
                                previousStepRootFileName,
                                (outputDir.replace(stepName, previousStepName)).replace(
                                    "/", "\/"
                                ),
                                jobInFileName,
                                jobOutFileName,
                        ...
                        os.system(
                            "sed -i 's/input = cms\.untracked\.int32(.*)/input = cms\.untracked\.int32(%s)/g' %s/%s"
                            % (evtPerJob, jobOutFileName, htcName)
                        )  # nuber of events
                    os.system(
                        "sed -i \"s/nevts:.*'),/nevts:%s'),/g\" %s/%s"
                        % (evtPerJob, jobOutFileName, htcName)
                    )  # nuber of events
                    # create scripts
                    prepareHTCOutputFolders()
                ...
                outputDir = outputDir.replace(
                    "/" + firstParName + str(fp) + "/", ""
                )  # remove the current firstPars or next firstPars will be inside it
            outputDir = outputDir.replace(
                "/" + secondParName + str(sp), ""
            )  # remove the current secondPars or next secondPars will be inside it
        outputDir = outputDir.replace(
            "/" + thirdParName + str(tp), ""
        )  # remove the current thirdPars or next thirdPars will be inside it
    ...
    for tp in thirdPars:
        ...
        for sp in secondPars:
            ...
            for fp in firstPars:
                ...
                if stepName == "wmLHEGEN":
                    gridpackFile = (
                        gridpackPath + process + "_" + parameters + "_" + archCMSSW
                    )
                    csvFileContent = (
                        parameters
                        + ","
                        ...
                        + ","
                        + gridpackFile
                    )  # NB nEvents and nJobs must be added to setup the csv file as expected, and not effect on how you split the events/jobs on HTcondor!
                else:
                    csvFileContent = (
                        parameters
                        + ","
                        ...
                        + "_"
                        + previousStepName
                    )  # NB these are placeholder values to comply with the EXO-MCsampleProductions framework. They are not meaningful
...
def defineParameters(tp, sp, fp):
    ...
    if sp != "none":
        if tp != "none":
            secondPar = "_" + secondParName + str(sp)
        else:
            secondPar = secondParName + str(sp)
    firstPar = ""
    if fp != "none":
        if sp != "none":
            firstPar = "_" + firstParName + str(fp)
        else:
            firstPar = firstParName + str(fp)
    else:
        sys.exit("which parameters are you running?")
...
def initializeProxy():
    ...
    os.system(
        "echo %s | voms-proxy-init --valid 192:00 -voms cms -rfc" % (passwd)
    )  # 192 means keep the credentials valid for 192h
...
def prepareHTCOutputFolders():
    if not os.path.exists("error"):
        os.makedirs(
            "error"
        )  # Do it here to avoid conflicts with !(*cfg) in prepareSubDataset
...
def prepareSHFileGridpack(jobOutFileName, process):
    ...
    path_at = (os.popen("pwd").read()).strip()  # .strip() removes empty characters
    print >> subFile, "pushd %sgenproductions/bin/MadGraph5_aMCatNLO/" % (
        outputFilePath
    )
    ...
    print >> subFile, "./gridpack_generation.sh %s cards/%s/%s/%s" % (
        jobOutFileName,
        ufoModelCase,
        process,
        jobOutFileName,
...
def prepareSHFile(jobOutFileName):
    ...
    print >> subFile, "popd"
    print >> subFile, "export X509_USER_PROXY=%s/x509up_u30997" % (path_at)
    print >> subFile, "export HOME=%s" % (
        homeDir
    )  # Need this line, if you do not want to fail w/ exit code 6 as explained here https://twiki.cern.ch/twiki/bin/view/CMSPublic/SWGuideCrabFaq
    print >> subFile, "voms-proxy-info -all"
    print >> subFile, "voms-proxy-info -all -file %s/x509up_u30997" % (path_at)
    print >> subFile, "cmsRun %s/%s.py" % (
        path_at + "/" + jobOutFileName,
        jobOutFileName,
    )
...
def prepareSubDataset(outputDir, isGridpack):
    ...
    if isGridpack:
        ...
        print >> subFile, 'transfer_output_remaps = "$Fn(filename).root=%s$Fn(filename).root"' % (
            outputDir
        )
